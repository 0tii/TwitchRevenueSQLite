...
def writeToDbLegacy(row, year, month, cur, all_results):
    """
    Sucky way of insert or update for sqlite installations without upsert funcionality (<3.24.0)\n
    Vastly less performant as an additional db-query and evaluation have to be performed\n
    ! optimizable by comparing to previous file date
    """
    sum = _arraySum(row)
    if sum <= 0 and not all_results: return

    cur.execute(f'''SELECT user_id FROM earnings WHERE user_id = {row[0]} AND year = {year} AND month = {month}''')

    if len(cur.fetchall()) == 0: #if PK exists, update
        #pk does not exist -> insert
        cur.execute(f'''INSERT INTO earnings (user_id, month, year, ad_share, sub_share, bit_share, bit_developer_share, bit_extension_share, prime_sub_share, bit_share_ad, fuel_rev, bb_rev, total_gross) 
                    VALUES({row[0]}, {month}, {year}, {row[2]}, {row[3]}, {row[4]}, {row[5]}, {row[6]}, {row[7]}, {row[8]}, {row[9]}, {row[10]}, {sum})''')
    else:
        #update
        cur.execute(f'''UPDATE earnings SET 
                    ad_share = ad_share + {row[2]}, 
                    sub_share = sub_share + {row[3]}, 
                    bit_share = bit_share + {row[4]},
                    bit_developer_share = bit_developer_share + {row[5]}, 
